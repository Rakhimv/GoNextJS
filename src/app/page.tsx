...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (email.trim() && name.trim()) {
      try {
        const req = await axios.post("http://localhost:6060/users", {
          "name": name,
          "email": email,
        });
        if (req.status === 200) {
  ...
  const getUsers = async () => {
    try {
      const req = await axios.get("http://localhost:6060/users");
      if (req.status === 200) {
        setUsers(req.data);
      }
    } catch {
  ...
  const deleteUser = async (id: number) => {
    try {
      const req = await axios.delete(`http://localhost:6060/users/${id}`);
      if (req.status === 200) {
        getUsers()
      }
    } catch (err) {
